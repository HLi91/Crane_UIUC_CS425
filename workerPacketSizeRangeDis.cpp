...
class packetSizeRangeDis : public ibolt
{
	ioCollector* _collector;
	mutex* writeLock;
	...
	void execute(myTuple tuple)
	{
		...
		if (myMap->find(tuple["time"]) == myMap->end()) {
			...
			result += "Distribution [10,100): " + to_string(count->at(0)) + " [100,1000): " + to_string(count->at(1)) + " [1000,10000): " + to_string(count->at(2)) + " [10000, Infinity): " + to_string(count->at(3));
			ofstream myfile;
			myfile.open("/home/jdeng5/mp4/data/cranePacketRangeDisOut.txt", std::ofstream::out | std::ofstream::app);
			myfile << result << "\n";
			myfile.flush();
			myfile.close();
			//tuple["result"] = result;
	...
	packetSizeRangeDis* clone()
	{
		packetSizeRangeDis* copy = new packetSizeRangeDis();
		copy->_collector = this->_collector;
...
int main(int argc, char *argv[])
{
	...
	builder->setBolt("count", new packetSizeRangeDis(), 1).shuffleGrouping("range");
